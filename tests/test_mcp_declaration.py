...
class TestTheRootDeclaresNoHostConfig(unittest.TestCase):
    ...
    def test_the_root_is_a_marketplace_not_a_plugin(self):
        """`.claude-plugin/` at the root holds the CATALOG. A `plugin.json` beside it declares the
        repo itself to be a plugin — which it stopped being on 2026-07-16, when the architecture
        became four plugins under `plugins/`.

        The file survived that change for eight days because nothing read it: not `build.py`, not a
        test, not CI. Verified at the docs rather than assumed — a marketplace entry's relative
        `source` "resolve[s] relative to the marketplace root, which is the directory containing
        `.claude-plugin/`", and ours are all `./plugins/<name>`, each with its own generated
        manifest. The root one is reachable by no documented path.

        Unread and hand-written is the worst pair in this repo: it had drifted to the old brand and
        the old repo URL, and the rebrand caught it by eye, which is not a mechanism. Deleted, and
        gated here so it cannot come back as a silently-wrong second answer to "what is this
        package called".
        """
        self.assertFalse(
            (ROOT / ".claude-plugin" / "plugin.json").exists(),
            "the repo root is the MARKETPLACE (.claude-plugin/marketplace.json, generated by "
            "build.py). A plugin.json beside it declares a fifth plugin that ships no skills, no "
            "agents and no hooks — and no host reads it. Per-plugin manifests are generated into "
            "plugins/*/.claude-plugin/plugin.json; that is the only manifest that is real.",
        )
