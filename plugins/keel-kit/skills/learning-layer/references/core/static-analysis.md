<!-- GENERATED FILE - do not edit. Source: src/core/static-analysis.md at the repo root; regenerate with: python scripts/build.py -->
...
- **Confidence from determinism — where the determinism is real.** A deterministic static finding
  ...
  that need it.
  The caveat is the whole reason `references/core/trust-axes.md` exists: this discount is earned by the
...
- **Degrade gracefully — but never silently.** No checker for a *present* language → the run
  ...
  **`coverage-gap`** pin (`confidence: extracted` — the absence is a fact). "Unchecked" must never
  ...
  graceful degradation looks like success. The general form lives in `references/core/trust-axes.md`, together
