# GENERATED FILE - do not edit. Source: src/runtime/fingerprint.py at the repo root;
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Signature fingerprints + change classifier for incremental re-audit "
                    "(the resume path). Compare against a prior fingerprints.json to get "
                    "SKIP/PARTIAL/ARCHITECTURE/FULL and the exact changed files.")
    parser.add_argument("root", nargs="?", default=".", help="repo root (default: .)")
    parser.add_argument("--against", help="prior fingerprints.json to diff against")
    parser.add_argument("-o", "--out", help="write the new fingerprints.json here (guarded save)")
    parser.add_argument("--commit", help="override built_at_commit (default: git HEAD)")
    parser.add_argument("--json", action="store_true", help="emit the verdict as JSON")
    args = parser.parse_args(argv)

    new_store = store(args.root, commit=args.commit)
    result: dict = {"files": len(new_store["files"]), "built_at_commit": new_store["built_at_commit"]}
    if args.against:
        old = load_store(args.against) or {"files": {}}
        changes = diff_stores(old, new_store)
        result["verdict"] = classify_update(changes, len(new_store["files"]))
    if args.out:
        wrote = save_store(new_store, args.out)
        result["wrote"] = wrote
        if not wrote:
            print("REFUSED: would overwrite a non-empty fingerprint store with an empty one.")
    if args.json or not args.against:
        print(json.dumps(result, ensure_ascii=False, indent=2))
    else:
        v = result["verdict"]
        print(f"update: {v['update']}  ({v['structural_count']} structural, "
              f"{len(v['cosmetic'])} cosmetic)")
