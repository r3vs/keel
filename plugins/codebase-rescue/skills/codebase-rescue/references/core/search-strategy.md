...
Shared doctrine for **locating** things in a tree someone else wrote. `references/core/static-analysis.md`
...
`extracted` confidence, and it never skips the false-positive gate (`references/core/trust-axes.md`).
...
keeping, it graduates from a `-p` one-liner to a YAML rule; `references/core/rule-authoring.md` is how one gets
...
already declared in `references/core/knowledge-sources.md`: the code, then authoritative docs, then a
...
the degraded answer is the same answer is not (`references/core/trust-axes.md`).
