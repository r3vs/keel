<!-- GENERATED FILE - do not edit. Source: src/core/rule-authoring.md at the repo root; regenerate with: python scripts/build.py -->
...
*with* rules is `references/core/search-strategy.md`; running them as a scanner whose output becomes pins is
`references/core/static-analysis.md`. This is the authoring step between them, and it exists because that step
...
- **Too specific, or the wrong language → matches nothing, exits 1.** A rule declared
  ...
  and it is the `references/core/static-analysis.md` rule turned structural: **a check that could not run found
  nothing, not zero.**
...
`references/core/knowledge-sources.md` (Context7 for authoritative library docs); ast-grep also publishes its
...
5. **Ship it only once step 3 passed.** A rule is not done when it is written; it is done when it
   ...
   (`references/core/trust-axes.md`) — never the one the author intended.
...
(`references/core/search-strategy.md`). Check the taxonomy per tool; never carry one tool's answer to the next.
