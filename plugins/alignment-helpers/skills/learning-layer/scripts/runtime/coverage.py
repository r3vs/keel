# GENERATED FILE - do not edit. Source: src/runtime/coverage.py at the repo root;
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Coverage manifest: which expected analysis capabilities ran vs are missing. "
                    "A gap is surfaced honestly, never hidden as a clean '0 findings'.")
    parser.add_argument("--langs", required=True,
                        help="comma-separated languages present (from tokei), e.g. Python,TypeScript")
    parser.add_argument("--reports", nargs="*", default=[],
                        help="SARIF/OSV report files that were actually produced")
    parser.add_argument("--ledger",
                        help="if given, write one coverage-gap incompleteness pin per gap and save")
    args = parser.parse_args(argv)

    langs = [l.strip() for l in args.langs.split(",") if l.strip()]
    rep = report(langs, args.reports)
    ...
    if args.ledger:
        ...
        pins = to_pins(led, rep["gaps"])
        led.save()
        print(f"coverage: {len(pins)} coverage-gap pin(s) written to {args.ledger}")

    print(f"coverage: {len(rep['ran'])} tool(s) ran, {rep['gap_count']} capability gap(s)")
    for g in rep["gaps"]:
        stacks = g["stacks"] if isinstance(g["stacks"], str) else ", ".join(g["stacks"])
        print(f"  [{g['severity']:>6}] GAP  {g['capability']} ({stacks}) — none of {g['tools']} ran")
    return 0
