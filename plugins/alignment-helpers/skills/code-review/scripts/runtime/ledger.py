...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Operate a ledger.json: read-only views PLUS the non-electing writes "
                    "(add a finding/defect, remediate, resolve). Electing a decision is the "
                    "human interview's job and is deliberately NOT exposed here — there is no "
                    "'decide' command, by design.")
    sub = parser.add_subparsers(dest="command", required=True)

    def _cmd(name: str, help_: str) -> "argparse.ArgumentParser":
        sp = sub.add_parser(name, help=help_)
        sp.add_argument("path", help="path to ledger.json")
        return sp

    # -- read-only views (unchanged) --
    _cmd("summary", "print pin / state / event counts")
    _cmd("interview", "list open needs_input questions, highest information-gain first")

    # -- non-electing writes (each persists atomically) --
    ap = _cmd("add-pin", "add a pin — a finding, a defect, an open_decision; never elects it")
    ap.add_argument("--kind", required=True, choices=sorted(KINDS))
    ap.add_argument("--title", required=True)
    ap.add_argument("--severity", required=True, choices=SEVERITIES)
    ap.add_argument("--confidence", required=True, choices=CONFIDENCES)
    ap.add_argument("--provenance", required=True,
                    help='JSON list, e.g. \'[{"source":"systematic-debugging","detail":"repro"}]\'')
    ap.add_argument("--as-is", help="JSON object — the current descriptive state (a defect's "
                                    "root cause goes here or in --kind-detail)")
    ap.add_argument("--to-be", help="JSON object — elected by the interview later, not here")
    ap.add_argument("--question", help="JSON object — materializes the pin as needs_input")
    ap.add_argument("--anchors", help="JSON list of code anchors")
    ap.add_argument("--depends-on", nargs="*", help="pin ids this depends on")
    ap.add_argument("--kind-detail", help="required when --kind other")
    ap.add_argument("--cluster-id")

    sa = _cmd("surface-assumption", "surface a forced assumption as a vetoable pin (anti-slop)")
    sa.add_argument("--title", required=True)
    sa.add_argument("--detail", required=True)
    sa.add_argument("--severity", default="medium", choices=SEVERITIES)
    sa.add_argument("--confidence", default="inferred", choices=("inferred", "ambiguous"))

    ar = _cmd("add-remediation", "attach a RemediationItem / BuildItem to a pin")
    ar.add_argument("--pin", required=True)
    ar.add_argument("--action", required=True)
    ar.add_argument("--ladder-rung", type=int, required=True)
    ar.add_argument("--canonical-target")
    ar.add_argument("--build-track", choices=("A", "B"))
    ar.add_argument("--contract-carrier")
    ar.add_argument("--depends-on", nargs="*")

    ss = _cmd("set-remediation-status", "move a remediation item todo -> in_progress -> done")
    ss.add_argument("--pin", required=True)
    ss.add_argument("--item", required=True)
    ss.add_argument("--status", required=True, choices=("todo", "in_progress", "done"))

    rs = _cmd("resolve", "resolve a pin — records the OBSERVED evidence ('resolved = observed')")
    rs.add_argument("--pin", required=True)
    rs.add_argument("--evidence", required=True,
                    help="what you OBSERVED that closed the gap — not 'the code is written'")

    df = _cmd("defer", "mark a pin out of scope now (future backlog)")
    df.add_argument("--pin", required=True)
    ...
    try:
        if args.command == "add-pin":
            pin = ledger.add_pin(
                kind=args.kind, title=args.title, severity=args.severity,
                confidence=args.confidence,
                provenance=_json_arg(args.provenance, "provenance"),
                anchors=_json_arg(args.anchors, "anchors"),
                as_is=_json_arg(args.as_is, "as-is"),
                to_be=_json_arg(args.to_be, "to-be"),
                question=_json_arg(args.question, "question"),
                depends_on=args.depends_on,
                cluster_id=args.cluster_id,
                kind_detail=args.kind_detail,
            )
            result = f"{pin['id']} ({pin['kind']}, state={pin['state']})"
        elif args.command == "surface-assumption":
            pin = ledger.surface_assumption(
                title=args.title, detail=args.detail,
                severity=args.severity, confidence=args.confidence)
            result = f"{pin['id']} (state={pin['state']})"
        elif args.command == "add-remediation":
            item = ledger.add_remediation(
                args.pin, action=args.action, ladder_rung=args.ladder_rung,
                canonical_target=args.canonical_target, build_track=args.build_track,
                contract_carrier=args.contract_carrier, depends_on=args.depends_on)
            result = f"{item['id']} on {args.pin} (status={item['status']})"
        elif args.command == "set-remediation-status":
            item = ledger.set_remediation_status(args.pin, args.item, args.status)
            result = f"{item['id']} -> {item['status']}"
        elif args.command == "resolve":
            pin = ledger.resolve(args.pin, evidence=args.evidence)
            result = f"{pin['id']} -> {pin['state']}"
        elif args.command == "defer":
            pin = ledger.defer(args.pin)
            result = f"{pin['id']} -> {pin['state']}"
        else:  # pragma: no cover — argparse forbids it
