...
(core stdlib-only, exercised by `tests/` on every PR): `ledger.py` (implements the spec, currently v0.24), `shapes.py` (field-shape engine +
...
- **The decisions ledger is the single source of truth.** Three surfaces — the visual map/wiki,
  ...
  the codebases they touch. Schema authority: `src/core/decisions-ledger-spec.md` (shared, v0.24);
...
- **Sources of truth:** each skill's `modules.json` is authoritative for its module catalog;
  ...
  `src/core/decisions-ledger-spec.md` (v0.24) is authoritative for the ledger schema. Do not let a
