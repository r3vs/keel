...
Every skill in this package except this one, `codebase-rescue` and `systematic-debugging` sets
`disable-model-invocation: true`, so nothing fires off a description any more: the operator types
`/test-driven-development`, `/prototype`, `/learning-layer`. The host loads a listing of skill names
and descriptions whose budget is 1% of the model's context window and drops descriptions starting
with the least-invoked skill — so a package spending the whole budget on seventeen entries loses
precisely the ones that must fire on a cold repo. `which-skill` is the map the operator reaches for
