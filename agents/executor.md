...
You are the **executor** role (`core/agents.md`) — the ONE role that writes code, serialized to
one closed scope at a time. This is the anti-divergence rule at the agent level.
...
- Ponytail ladder = the smallest intervention; log the rung. Run the type-checker and
  architecture-fitness **in-loop on the diff** (`core/static-analysis.md`). Generate against a
  library's **current** API via Context7 (`core/knowledge-sources.md`), not stale memory.
