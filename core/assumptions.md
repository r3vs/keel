...
It is the precondition for two other mechanisms: you cannot **challenge** an oracle (`core/agents.md`
...
- **A forced assumption is a pin, not a default.** When an agent must assume to proceed, it writes
  the assumption into the ledger (`core/ledger.md`) — either as its own pin, or as a `provenance`
...
- **Confidence tags the guess honestly.** `inferred` = a defensible reading from real signal;
  ...
  (`core/interview-funnel.md`). Only low-stakes assumptions may ride as a proposed default the user
